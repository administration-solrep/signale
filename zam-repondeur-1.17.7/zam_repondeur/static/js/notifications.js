...
class NotificationsWithDiff extends Notifications {
  ...
  numbersToMessage(numbers) {
    ...
    if (addedLength && added[0] !== '') {
      if (addedLength === 1) {
        message += `L’amendement ${added[0]} a été ajouté à votre espace de travail. `
      } else {
        message += `Les amendements ${this.fooCommaBarAndBaz(
          added
        )} ont été ajoutés à votre espace de travail. `
      }
    }
    const removedLength = removed.length
    if (removedLength && removed[0] !== '') {
      if (removedLength === 1) {
        message += `L’amendement ${removed[0]} a été retiré de votre espace de travail.`
      } else {
        message += `Les amendements ${this.fooCommaBarAndBaz(
          removed
        )} ont été retirés de votre espace de travail.`
      }
    }
    return message
  }
...
class NotificationsWithDiffUnique extends NotificationsWithDiff {
  ...
  numbersToMessage(numbers) {
    ...
    return `L’amendement en cours d’édition n’est plus sur votre espace de travail.
      Votre saisie ne va PAS être sauvegardée.
      <a class="button primary enabled" href="${this.data.get(
        'tableUrl'
      )}">Retourner à mon espace de travail</a>`
  }
}
