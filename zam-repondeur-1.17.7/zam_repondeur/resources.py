...
from zam_repondeur.models.events.dossiers_list import DossiersListEvent
...
class Root(Resource):
    __acl__ = [
        ...
        (Allow, "group:admins", "manage_actu"),
        (Deny, Everyone, "manage_actu"),
    ]
    ...
    def __init__(self, _request: Request) -> None:
        ...
        self.add_child(TechniqueCollection(name="technique", parent=self))
...
class TechniqueCollection(Resource):
    __acl__ = [(Allow, "group:admins", "manage"), (Deny, Everyone, "manage")]
...
class DossierCollection(Resource):
    ...
    def events(self) -> Query:
        return DBSession.query(DossiersListEvent).order_by(
            desc(DossiersListEvent.created_at)
        )
...
class DossierResource(Resource):
    def __acl__(self) -> List[ACE]:
        ...
        return [
            ...
            (Allow, f"team:{self.dossier.team.pk}:True", "active"),
            (Deny, f"team:{self.dossier.team.pk}:False", "active"),
            (Allow, f"group:admins", "gestion"),
            (Deny, Authenticated, "gestion"),
            (Allow, f"group:admins", "retrait"),
            (Deny, Authenticated, "retrait"),
            (Allow, f"group:admins", "set_contributeur"),
            (Deny, Authenticated, "set_contributeur"),
            (Allow, f"group:admins", "set_coordinateur"),
            (Deny, Authenticated, "set_coordinateur"),
            (Allow, f"group:admins", "set_lecture_update"),
            (Deny, Authenticated, "set_lecture_update"),
            (Allow, f"group:admins", "archive_dossier"),
            (Deny, Authenticated, "archive_dossier"),
            (Allow, f"group:admins", "set_dossier_alert"),
            (Deny, Authenticated, "set_dossier_alert"),
        ]
    ...
    def __init__(self, name: str, parent: Resource) -> None:
        ...
        self.add_child(ImportExportDossier(name="import_export", parent=self))
    ...
    def model(self, *options: Any) -> Dossier:
        ...
        if dossier is None or dossier.team is None:
            raise ResourceNotFound(self)
        return dossier


class ImportExportDossier(Resource):
    def __init__(self, name: str, parent: Resource) -> None:
        super().__init__(name=name, parent=parent)
...
class LectureResource(Resource):
    def __init__(
        ...
        super().__init__(name=name, parent=parent)
        self.slug = self.dossier_resource.slug
        ...
        self.add_child(RechercheCollection(name="recherche", parent=self))
        self.add_child(ArticleCollection(name="articles", parent=self))
        self.add_child(TableCollection(name="tables", parent=self))
        self.add_child(SharedTableCollection(name="corbeilles", parent=self))
        self.add_child(ImportExportLecture(name="import_export", parent=self))
...
class ImportExportLecture(Resource):
    def __init__(self, name: str, parent: Resource) -> None:
        super().__init__(name=name, parent=parent)
...
class TableResource(Resource):
    def __init__(self, name: str, parent: Resource) -> None:
        super().__init__(name=name, parent=parent)
        self.add_child(TagsCollection(name="tags", parent=self))
...
class TagsCollection(Resource):
    ...
    def __acl__(self) -> List[ACE]:
        # Only team members and admins can view it.
        return [
            (Allow, f"user:{self.owner.pk}", "view"),
            (Deny, Authenticated, "view"),
        ]

    @property
    def parent(self) -> TableResource:
        return cast(TableResource, self.__parent__)

    @property
    def user_table_resource(self) -> TableResource:
        return self.parent

    @property
    def owner(self) -> User:
        try:
            user: User = DBSession.query(User).filter(
                User.email == self.parent.__name__
            ).one()
            return user
        except NoResultFound:
            raise ResourceNotFound
...
class RechercheCollection(Resource):
    def __getitem__(self, key: str) -> Resource:
        return RechercheResource(name=key, parent=self)
...
class RechercheResource(Resource):
    ...
    @property
    def parent(self) -> RechercheCollection:
        return cast(RechercheCollection, self.__parent__)
