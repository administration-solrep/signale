...
def download_akn_xml(url: str) -> str:
    ...
    if resp.status_code != HTTPStatus.OK:
        logger.warning(f"Impossible de récupérer le texte AKN {url} du SENAT")
        raise AlertOnData(
            f"Impossible de récupérer le texte AKN {url} du SENAT \
...
def get_articles(akn_content: str) -> List[Dict]:
    ...
    # On récupére toutes les balise articles sans classe
...
def extract_alinea_pastillage(alinea: element.Tag) -> str:
    ...
    if alinea.isSelfClosing and DATA_MENTION not in attributes:
        guid = alinea.get("GUID", "")
        logger.error("Self closing alinea tag {}".format(guid))
        raise AlertOnData(
            f"Problème lors de la récupération de la pastille ref:{guid}", "data", 1
        )
