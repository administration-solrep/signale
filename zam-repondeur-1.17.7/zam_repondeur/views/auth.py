...
from zam_repondeur.mails import get_badge
from zam_repondeur.message import Message
from zam_repondeur.models import DBSession, User, get_one_or_create
from zam_repondeur.resources import AmendementResource, ImportExportLecture, Root
...
class UserLogin(RateLimiterMixin):
    ...
    def send_auth_token_email(self, token: str, email: str) -> None:
        ...
        reply_to = settings["zam.contact_mail"]
        contact_mail_from = settings["zam.contact_mail_from"]
        token_duration = int(int(settings["zam.users.auth_token_duration"]) / 60)
        mailer = get_mailer(self.request)
        subject = get_badge(self.request.registry) + "Se connecter à Signale"
        message = MailMessage(
            subject=subject,
...
Pour vous connecter à Signale, veuillez cliquer sur l’adresse suivante :
{url}

Notez que Signale ne fonctionne qu'avec des versions de navigateurs récents
tels que Mozilla Firefox (version 74.0 ou supérieur), Microsoft Edge
(version 88.0 ou supérieur) ou Google Chrome (version 80.0 ou supérieur).
A l'inverse, l'utilisation du navigateur Internet Explorer
doit être proscrite, ce navigateur étant obsolète.

Ce lien contient un code personnel à usage unique,
valable {token_duration} minutes, pour vous authentifier sur Signale.

Une fois connecté, vous pourrez directement accéder aux dossiers :
{url_dossiers}
...
class Authenticate(RateLimiterMixin):
    ...
    def get(self) -> Any:
        ...
        self.request.session.flash(
            Message(cls="success", text="Bienvenue dans Signale !")
        )
...
class Welcome:
    ...
    def post(self) -> Any:
        ...
        clean_name = clean_all_html(name)
        self.request.user.name = User.normalize_name(clean_name)
...
def forbidden_view(exception: HTTPForbidden, request: Request) -> Any:
    ...
    if isinstance(exception.result, ACLDenied):
        ...
        elif acl_denied.permission == "active":
            message = "Ce dossier est en lecture seule."
            if "amendements" in request.context.keys():
                next_resource = request.context["amendements"]
            elif request.context.__class__ == AmendementResource:
                next_resource = request.context.parent
            elif request.context.__class__ == ImportExportLecture:
                next_resource = request.context.parent["amendements"]
            else:
                next_resource = request.context
        elif acl_denied.permission in [
            "gestion",
            "set_coordinateur",
            "set_contributeur",
        ]:
            message = "Vous n’êtes pas autorisé à gérer l’équipe de ce dossier."
            next_resource = request.context
        elif acl_denied.permission == "retrait":
            message = "Vous n’êtes pas autorisé à retirer une personne de ce dossier."
            next_resource = request.context
        elif acl_denied.permission == "manage":
