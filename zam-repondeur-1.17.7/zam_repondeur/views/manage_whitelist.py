...
from zam_repondeur.data_sanitize import get_as_int_or_none, sanitize_string
...
class WhitelistDelete(WhitelistCollectionBase):
    ...
    def post(self) -> Response:
        email_pattern_pk: Optional[int] = get_as_int_or_none(self.request.POST["pk"])
        if email_pattern_pk is None:
...
class WhitelistAddForm(WhitelistCollectionBase):
    ...
    @view_config(request_method="POST")
    def post(self) -> Response:
        email_pattern = sanitize_string(self.request.POST["email_pattern"] or "")
