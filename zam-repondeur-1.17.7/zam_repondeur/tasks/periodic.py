...
from zam_repondeur.tasks.fetch import update_dossier, update_dossier2texte
from zam_repondeur.tasks.huey import huey

logger = logging.getLogger(__name__)

CRON_UPDATE_DATA = huey.settings.get("zam.cron.update_data") or "1 * * * *"
CRON_UPDATE_DOSSIERS = huey.settings.get("zam.cron.update_dossiers") or "10 * * * *"
CRON_UPDATE_DOSSIERS2TEXTE = (
    huey.settings.get("zam.cron.update_dossiers2textes") or "15 * * * *"
)


@huey.periodic_task(crontab(*CRON_UPDATE_DATA.split(" ")))
def update_data() -> None:
    update_data_repository()
    update_textes()
    update_dossiers()
    update_dossiers2textes()
...
def create_missing_dossiers_an() -> None:
    ...
    for an_id in missing_an_ids:
        ...
        # Mantis 13799: Cette partie bloque la création de certains dossiers
...
def update_dossiers2textes() -> None:
    for dossier in DBSession.query(Dossier).all():
        update_dossier2texte.schedule(args=(dossier.pk,), delay=1)
...
@huey.periodic_task(crontab(*CRON_UPDATE_DOSSIERS2TEXTE.split(" ")))
def update_all_dossiers2textes() -> None:
    for dossier in DBSession.query(Dossier):
        delay = dossier.pk % 10  # spread out updates over 10 sec
        update_dossier2texte.schedule(args=(dossier.pk,), delay=delay)


# Keep it last as it takes time and will add up with the growing number of dossiers.
@huey.periodic_task(crontab(*CRON_UPDATE_DOSSIERS.split(" ")))
def update_all_dossiers() -> None:
    for team in DBSession.query(Team).filter(Team.dossier_pk.isnot(None), Team.active):
