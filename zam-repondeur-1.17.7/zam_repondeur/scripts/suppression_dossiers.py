...
des dossiers sur cet environnement"
    ...
    with bootstrap(args.config_uri, options={"app": "zam_suppression_dossiers"}):
        with transaction.manager:
            for team in DBSession.query(Team).all():
                logging.info(f"Supression du dossier {team.dossier.titre}")
                for lecture in team.dossier.lectures:
                    logging.info(f"Supression de la lecture {lecture} et son journal")
                    DBSession.delete(lecture)
                logging.info(f"Supression du journal de {team.dossier.titre}")
                for event in team.dossier.events:
