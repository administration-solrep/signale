...
def main(argv: List[str] = sys.argv) -> None:
    ...
    with bootstrap(args.config_uri, options={"app": "zam_load_data"}) as env:
        if args.command == "grant":
            grant_admin(email=args.email, request=env["request"])
        elif args.command == "list":
            list_admins()
        elif args.command == "revoke":
            revoke_admin(email=args.email, request=env["request"])
        else:
            sys.exit(1)
...
def grant_admin(email: str, request: Request) -> None:
    with transaction.manager:
        ...
        send_manage_admins(is_grant=True, request=request, user=user)
...
def revoke_admin(email: str, request: Request) -> None:
    with transaction.manager:
        user = DBSession.query(User).filter(User.email == email).first()
        if not user:
            print(f"Email {email} is not registered", file=sys.stderr)
        ...
        send_manage_admins(is_grant=False, request=request, user=user)
