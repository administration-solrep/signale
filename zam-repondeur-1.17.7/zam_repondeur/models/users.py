...
coordinator_association_table = Table(
    "teams2coordinators",
    Base.metadata,
    Column("team_pk", Integer, ForeignKey("teams.pk"), primary_key=True),
    Column("user_pk", Integer, ForeignKey("users.pk"), primary_key=True),
...
class Team(Base):
    ...
    coordinators = relationship("User", secondary="teams2coordinators")
    ...
    def is_coordinator(self, user: "User") -> bool:
        return user in self.coordinators

    def add_members(self, users: Iterable["User"]) -> None:
        for user in users:
            self.users.append(user)

    def add_coordinators(self, users: Iterable["User"]) -> None:
        for user in users:
            self.coordinators.append(user)
            self.users.append(user)

    def remove_member(self, user: "User") -> None:
        if user in self.users:
            self.users.remove(user)
        if user in self.coordinators:
            self.coordinators.remove(user)

    def remove_coordinateur(self, user: "User") -> None:
        if user in self.coordinators:
            self.coordinators.remove(user)
    ...
    def every_coordinator_but_me(self, me: "User") -> List["User"]:
        return [user for user in self.coordinators if user is not me]

    def every_lambda_user_but_me(self, me: "User") -> List["User"]:
        return [
            user
            for user in self.users
            if user not in self.coordinators and user is not me
        ]
...
class User(Base):
    ...
    @classmethod
    def get_role(cls, user: "User", team: "Team") -> str:
        if user.is_admin and team.is_coordinator(user):
            return "Administrateur et Coordinateur"
        if user.is_admin:
